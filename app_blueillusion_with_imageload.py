...
def load_embedding():
    ...
    return OpenAIEmbeddings(model=Config.EMBEDDING_MODEL, api_key=OPENAI_API_KEY)
...
If the user has not asked a question related to fashion and Blue Illusion products, you can respond with 'I do not have the products you asked for' and suggest a simialr alternative from BlueIllusion context.
...
with st.sidebar:
    ...
    memory = load_memory()
    prompt = load_prompt()
    image_prompt = load_image_prompt()
...
with st.form('load_files'):
    ...
    submitted = st.form_submit_button('Match this image')
    if submitted:
        ...
        # Add the prompt to messages, stored in session state
            question = st.session_state.image_description
            ...
            with st.chat_message("assistant"):
                ...
                chain = inputs | image_prompt | model
